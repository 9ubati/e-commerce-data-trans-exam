package com.datatrans.ecommerce.orderms.common;
...
public class ApiExceptionHandler {
	...
	public ResponseEntity<?> handleInvalidParameters(InvalidParameters ex) {
		return ResponseEntity.status(HttpStatus.BAD_REQUEST)
				.body(Map.of("timestamp", OffsetDateTime.now().toString(), "status", 400, "error", "InvalidParameters",
						"message", "One or more parameters are invalid.", "details", ex.getErrors() // comes from your
																									// custom exception
				));
	}
